...
class ManualMilkPage extends StatefulWidget {
  const ManualMilkPage({Key? key}) : super(key: key);

  @override
  State<ManualMilkPage> createState() => _ManualMilkPageState();
}

class _ManualMilkPageState extends State<ManualMilkPage> {
  void initState() {
    ...
    return Consumer<AutomaticProvider>(builder: (context, provider, child) {
      ...
      return PopScope(
          ...
          child: Main_Theme.mainTheme(
              ...
              pageName: itemText.manualMilkCollection,
              widget: SingleChildScrollView(
                child: Column(
                  children: [
                                    ...
                                    child: ButtonTheme(
                                      ...
                                      child: DropdownButton<String>(
                                        ...
                                        items: [
                                          ...
                                          ...provider.FarmerList.map((item) {
                                            return new DropdownMenuItem(
                                              child: new Text(
                                                  item.name.toString(),
                                                  style: TextStyle(
                                                      color: AppColor.black)),
                                              value: item.userUniId.toString(),
                                            );
                                          }).toList()
                                        ],
                                      ),
                            ...
                            Container(
                              ...
                              child: TextField(
                                ...
                                decoration: InputDecoration(
                                    ...
                                    contentPadding: EdgeInsets.only(
                                        bottom: 10, top: 10, left: 4, right: 4),
                                    floatingLabelBehavior:
                                        FloatingLabelBehavior.always,
                                    border: OutlineInputBorder(
                                        borderSide: BorderSide.none,
                    ...
                    InkWell(
                      onTap: () {
                        if (provider.nameControler.text.trim().isEmpty) {
                          snackMessage(context,
                              "${itemText.pleaseEnter} ${itemText.name}");
                        } else if (provider.dateControler.text.trim().isEmpty) {
                          snackMessage(context,
                        ...
                        } else if (provider.snfController.text.trim().isEmpty) {
                          snackMessage(context,
                              "${itemText.pleaseEnter} ${itemText.snf}");
                        } else if (provider.qtyControler.text.trim().isEmpty) {
                          snackMessage(context,
                              "${itemText.pleaseEnter} ${itemText.qty}");
                        } else if (provider.clrController.text.trim().isEmpty) {
                          snackMessage(context,
                              "${itemText.pleaseEnter} ${itemText.clr}");
                        } else if (provider.rateControler.text.trim().isEmpty) {
                          snackMessage(context,
