...
class DisReport extends StatefulWidget {
  const DisReport({Key? key}) : super(key: key);
              ...
              children: [
                ...
                InkWell(
                  onTap: () async {
                    ...
                    } else if (provider.shift.toString().isEmpty ||
                        provider.shift == null ||
                        provider.shift == itemText.select) {
                      snackMessage(
                          context, "${itemText.pleaseEnter} ${itemText.shift}");
                    } else if (provider.myItemList == itemText.select) {
                      snackMessage(context,
                          "${itemText.pleaseEnter} ${itemText.milkType}");
                    } else {
                      var response = await provider.GetDispatchReportList();
                      if (response['success'] == true &&
                          response['data'] != "" &&
                          response['data'] != null &&
                          !response['data'].isEmpty) {
                        DispatchReportModel rateChatModel =
                            DispatchReportModel.fromJson(response);
                        provider.dispatchReport.addAll(rateChatModel.data!);
                        snackMessage(context, response["message"], no: 2);
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => DispatchReportShowList(),
                            ));
