...
class DisMilkSlip extends StatefulWidget {
  const DisMilkSlip({Key? key}) : super(key: key);

  @override
  State<DisMilkSlip> createState() => _DisMilkSlipState();
}

class _DisMilkSlipState extends State<DisMilkSlip> {
  @override
  void initState() {
                    ...
                    InkWell(
                      onTap: () async {
                        if (provider.shift.toString().isEmpty ||
                          ...
                          var response = await provider.GetDispatchSlipList();
                          if (response['success'] == true &&
                              response['data'] != "" &&
                              response['data'] != null &&
                              !response['data'].isEmpty) {
                            snackMessage(context, response["message"], no: 2);
                            DispatchModel rateChatModel =
                                DispatchModel.fromJson(response);
                            List<DispatchModelData> data = [];
                            data = rateChatModel.data!;
                            provider.dispatchSlip.addAll(data);
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => DispatchSlipShowList(),
                                ));
